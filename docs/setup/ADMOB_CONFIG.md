...
`google_mobile_ads`はDart側の広告ユニットIDだけでなく、iOS/Androidのネイティブ設定にもAdMob App IDが必要です。

- iOS: `ios/Runner/Info.plist` の `GADApplicationIdentifier` は `$(GAD_APPLICATION_IDENTIFIER)` を参照し、Xcode Build Settingsでdev/prodを切り替えます。
- Android: `android/app/src/main/AndroidManifest.xml` の `com.google.android.gms.ads.APPLICATION_ID` は `${adMobApplicationId}` を参照し、Gradle flavorの `manifestPlaceholders` でdev/prodを切り替えます。

設定漏れやDart側との不一致は以下のテストで確認します。

```bash
fvm flutter test \
  test/utils/android_admob_app_id_config_test.dart \
  test/utils/ios_admob_app_id_config_test.dart \
